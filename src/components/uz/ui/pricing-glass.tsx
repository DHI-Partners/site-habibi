...
export type TierType = {
  ...
  description: string;
  isPopular?: boolean;
  /** Dark card with a gold border (premium plan without a price). */
  isExclusive?: boolean;
  features: string[];
  /** Explicitly NOT included items — rendered with an X and dimmed. */
  excludedFeatures?: string[];
  /** If set — this label is shown instead of price/currency/"/mo"
   *  (e.g. "After audit" for a plan without a fixed price). */
  priceLabel?: string;
  /** Custom CTA button label (otherwise the shared ctaLabel is used). */
  ctaLabel?: string;
  /** Hides the trial note under the CTA (for tiers without a free trial). */
  noTrial?: boolean;
};

type BillingPeriod = "monthly" | "semi" | "annual";

/** Вторая валюта: включает переключатель и пересчёт цен по курсу. */
export interface AltCurrency {
  /** Подпись кнопки переключателя, например «soʻm». */
  label: string;
  /** Суффикс после суммы, например «soʻm». */
  suffix: string;
  /** Курс: 1 базовая валюта = rate альтернативных. */
  rate: number;
  /** Шаг округления суммы, чтобы цена выглядела прайс-листом (например 10000). */
  round?: number;
  /** Мелкая подпись под переключателем — курс ориентировочный. */
  note?: string;
}
...
export interface PricingGlassProps {
  ...
  tiers: TierType[];
  className?: string;
  /** CTA button text on the card. */
  ctaLabel?: string;
  /** Currency symbol before the price (default $). */
  currency?: string;
  /** Подпись кнопки основной валюты в переключателе (например «EUR»). */
  currencyLabel?: string;
  /** Если задано — рядом с периодом появляется переключатель валют. */
  altCurrency?: AltCurrency;
  /** Free-trial badge shown in the section header. */
  trialBadge?: string;
...
/** Разряды через пробел: 2 770 000. */
...
function PricingCard({
  ...
  ctaLabel: string;
  currency: string;
  /** Выбранная альтернативная валюта (null — показываем базовую). */
  alt: AltCurrency | null;
  trialNote?: string;
  onGetStarted?: (tier: TierType) => void;
}) {
  const price =
  ...
  // Savings for the selected period (in currency), relative to monthly billing.
  ...
  const savingsLabel = period === "semi" ? "Yarim yilda" : "Yiliga";

  /** Пересчёт в альтернативную валюту с округлением до «прайсового» шага. */
  const convert = (value: number) => {
    if (!alt) return value;
    const step = alt.round || 1;
    return Math.round((value * alt.rate) / step) * step;
  };

  // Экономию считаем из уже округлённых сумм — иначе она не сходится с показанными ценами.
  const savings = alt
    ? (convert(monthlyNum) - convert(currentNum)) * months
    : Math.round((monthlyNum - currentNum) * months);

  /** Цена внутри пункта тарифа: {{500}} → «€500» либо «6 950 000 soʻm». */
  const renderFeature = (text: string) =>
    text.replace(/\{\{(\d+(?:\.\d+)?)\}\}/g, (_, raw: string) =>
      alt
        ? `${groupDigits(convert(parseFloat(raw)))} ${alt.suffix}`
        : `${currency}${raw}`,
    );
      ...
      <div className="relative z-10 flex flex-col p-8 md:p-10 flex-1 pointer-events-none">
        ...
        <motion.div variants={legoVariant} className="flex items-baseline gap-1 mt-4 mb-2">
          {tier.priceLabel ? (
            ...
            /* Сумма в сумах длиннее — шрифт меньше, валюта суффиксом после числа. */
            <>
              <div className="h-[60px] overflow-hidden flex items-center">
                <span
                  key={`alt-${price}`}
                  className="block text-[32px] md:text-[38px] font-bold text-white tracking-tight leading-none [animation:priceIn_0.4s_cubic-bezier(0.16,1,0.3,1)]"
                >
                  {groupDigits(convert(currentNum))}
                </span>
              </div>
              <span className="text-white/40 text-base font-medium">{alt.suffix}</span>
              <span className="text-white/40 text-base font-medium ml-0.5">/oy</span>
            </>
          ) : (
            <>
              ...
              <span className="text-white/40 text-lg font-medium ml-1">/oy</span>
            </>
          )}
        </motion.div>

        {/* Savings for the selected period — reserve height so the layout doesn't jump */}
        <div className="mb-3 h-7">
          {!tier.priceLabel && savings > 0 && (
            ...
            >
              {alt
                ? `${savingsLabel} ${groupDigits(savings)} ${alt.suffix} tejaysiz`
                : `${savingsLabel} ${currency}${savings} tejaysiz`}
            </span>
          )}
        </div>
        ...
        <div className="flex flex-col gap-4 mb-10 flex-1">
          {tier.features.map((feat: string, i: number) => (
            <motion.div key={i} variants={legoVariant} className="flex items-start gap-3">
              ...
              <span className="text-white/70 font-medium text-[14px] leading-tight">{renderFeature(feat)}</span>
            </motion.div>
          ))}
...
export function PricingGlass({
  title = "Sodda va shaffof narxlar.",
  description = "Mos tarifni tanlang. Yillik toʻlov 20% gacha tejaydi.",
  tiers,
  className,
  ctaLabel = "Boshlash",
  currency = "$",
  ...
  const alt = altCurrency && useAlt ? altCurrency : null;
    ...
    >
      ...
      <div className="flex flex-col items-center gap-8 relative z-20 w-full">
        ...
        <motion.div variants={legoVariant} className="relative flex w-full max-w-md items-center rounded-full border border-white/10 bg-white/5 p-1.5 shadow-[inset_0_1px_4px_rgba(0,0,0,0.5)] backdrop-blur-3xl">
          {/* Sliding indicator — one third of the width */}
          <motion.div
        ...
        {/* Переключатель валют: евро ↔ сумы по курсу */}
        {altCurrency && (
          <motion.div variants={legoVariant} className="flex flex-col items-center gap-2 -mt-4">
            <div className="relative flex items-center rounded-full border border-white/10 bg-white/5 p-1 shadow-[inset_0_1px_4px_rgba(0,0,0,0.5)] backdrop-blur-3xl">
              <motion.div
                className="absolute left-1 top-1 bottom-1 w-[calc((100%-0.5rem)/2)] rounded-full border border-white/20 bg-white/10 shadow-[0_2px_8px_rgba(0,0,0,0.5)]"
                animate={{ x: useAlt ? "100%" : "0%" }}
                transition={{ type: "spring", stiffness: 400, damping: 30 }}
              />
              <button
                onClick={() => setUseAlt(false)}
                className={`relative z-10 min-w-[76px] rounded-full px-4 py-2 text-xs font-semibold transition-colors duration-300 ${!useAlt ? "text-white" : "text-white/50 hover:text-white/80"}`}
              >
            ...
            {useAlt && altCurrency.note && (
              <p className="max-w-md px-4 text-center text-[11px] leading-relaxed text-white/35 [animation:fadeSlideUp_0.3s_ease_both]">
                {altCurrency.note}
              </p>
            )}
          </motion.div>
      ...
      <div className="relative w-full grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-4 gap-6 lg:gap-8 items-stretch z-20">
        {tiers.map((tier) => (
          <PricingCard key={tier.name} tier={tier} period={period} ctaLabel={ctaLabel} currency={currency} alt={alt} trialNote={trialNote} onGetStarted={onGetStarted} />
        ))}
      </div>
