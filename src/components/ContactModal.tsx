...
interface ContactModalProps {
  ...
  /** Своя тема письма (иначе — «Новая заявка с сайта Habibi — тариф …»). */
  subject?: string
  /** Свой подзаголовок формы (иначе — «Тариф X. Заполните контакты…»). */
  heading?: string
}
    ...
    try {
      const res = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          subject: subject ?? `Новая заявка с сайта Habibi — тариф ${tierName || '—'}`,
      ...
      >
        ...
        {!submitted ? (
          ...
          <form onSubmit={handleSubmit}>
            ...
            <label className="mt-4 block">
              <span className="mb-1.5 block text-sm font-medium text-white/70">Комментарий</span>
              <div className="flex items-start gap-2.5 rounded-xl border border-white/10 bg-white/[0.04] px-3.5 focus-within:border-white/30">
                ...
                <textarea
                  ...
                  placeholder="Расскажите коротко, что нужно"
                  rows={3}
            ...
            <p className="mt-3 text-center text-xs leading-relaxed text-white/35">
              Нажимая кнопку, вы соглашаетесь на обработку контактных данных в соответствии с 
              <a href="/ru/privacy" target="_blank" rel="noopener noreferrer" className="text-white/55 underline underline-offset-2 transition-colors hover:text-white">
                Политикой конфиденциальности
              </a> 
              и принимаете 
              <a href="/ru/terms" target="_blank" rel="noopener noreferrer" className="text-white/55 underline underline-offset-2 transition-colors hover:text-white">
          ...
          <div className="py-4 text-center [animation:fadeSlideUp_0.4s_ease_both]">
            ...
            <p className="mx-auto mt-3 max-w-xs text-sm leading-relaxed text-white/60">
              ...
              {channel === 'telegram' ? ' в Telegram' : channel === 'whatsapp' ? ' в WhatsApp' : ''}.
            </p>
            <LiquidButton
              size="lg"
              onClick={onClose}
