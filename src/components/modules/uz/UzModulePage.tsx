...
import Footer from '../../uz/Footer'
import AiChatWidget from '../../uz/AiChatWidget'
import { Reveal } from '../../Reveal'
import { UZ_MODULE_PAGES } from './data'
import type { ModulePageData } from '../types'

/* Страница модуля узбекской версии (/uz/modules/:slug). Зеркалит EnModulePage:
   узбекские тексты интерфейса + живое HTML-превью вместо скриншота. */
...
function UzModulePageContent({ data }: { data: ModulePageData }) {
  const { open } = useContact()

  useEffect(() => {
    document.title = `${data.title} — Habibi ERP moduli`
    window.scrollTo(0, 0)
    return () => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#12121a] font-geist text-white">
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16 pt-14 text-center md:px-12 md:pb-24 md:pt-20">
        <Reveal>
          <SectionTag>Habibi ERP moduli</SectionTag>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        <Reveal>
          <SectionTag>Muammo</SectionTag>
          <h2 className="text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">Tanishmi?</h2>
        </Reveal>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        <Reveal>
          <SectionTag>Tartibsizlik narxi</SectionTag>
          <h2 className="text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">
            Bu biznesingizga qanchaga tushadi
          </h2>
      ...
      {/* Живое превью интерфейса */}
      {(data.Preview || data.screenshot) && (
        <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
          <Reveal>
            <SectionTag>Interfeys</SectionTag>
            <h2 className="text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">
              {data.previewTitle ?? data.screenshot?.title ?? `Habibiʼda «${data.title}»`}
            </h2>
          </Reveal>
          <Reveal delay={0.08}>
            {data.Preview ? (
              ...
              <img
                ...
                alt={`Habibiʼda «${data.title}» moduli interfeysi`}
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        <Reveal>
          <SectionTag>Natija</SectionTag>
          <h2 className="text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">
            Modul hal qiladigan muammolar
          </h2>
        </Reveal>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        <Reveal>
          <SectionTag>Eng muhimi</SectionTag>
          <h2 className="text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">
            Modul koʻproq daromad olishga qanday yordam beradi
          </h2>
        </Reveal>
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        <Reveal>
          <SectionTag>Habibiʼda tez orada</SectionTag>
          <h2 className="text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">
            {data.ai.title}
          </h2>
          <p className="mt-4 max-w-3xl text-base leading-relaxed text-white/60 md:text-lg">
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        <Reveal>
          <SectionTag>Ekotizim</SectionTag>
          <h2 className="text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">
            Boshqa modullar bilan birga kuchliroq
          </h2>
          <p className="mt-4 max-w-3xl text-base leading-relaxed text-white/60 md:text-lg">
            Habibi — oʻnta tarqoq dastur oʻrniga yagona tizim. Savdo omborni koʻradi, ombor
            xaridlarni koʻradi — siz esa butun biznesni koʻrasiz.
          </p>
        </Reveal>
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        <Reveal>
          <SectionTag>Kimlar uchun</SectionTag>
          <h2 className="text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">
            Biznesingizga mos keladi
          </h2>
        </Reveal>
        <Reveal delay={0.06}>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        <Reveal>
          <SectionTag>Tariflar</SectionTag>
          <h2 className="text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">
          ...
          {[
            {
              name: 'Habibi',
              price: '€19/oy',
              period: 'yillik toʻlovda oyiga €13 dan',
              features: data.tariff.baseIncludes
                ? [`${data.title} — tarifga kiradi`, 'CRM, loyihalar, sayt, moliya', '10 tagacha foydalanuvchi']
                : ['Mijozlar va savdo', 'Loyihalar va vazifalar, sayt, moliya', '10 tagacha foydalanuvchi'],
              highlight: data.tariff.baseIncludes,
            },
            {
              name: 'Habibi Pro',
              price: '€49/oy',
              period: 'yillik toʻlovda oyiga €34 dan',
              features: ['Barcha 10 modul va AI-agentlar', '50 tagacha foydalanuvchi va 5 integratsiya', 'Bazaviy texnik yordam'],
              highlight: !data.tariff.baseIncludes,
            },
            {
              name: 'Habibi Premium',
              price: '€199/oy',
              period: 'yillik toʻlovda oyiga €139 dan',
              features: ['Barcha modullar va kengroq limitlar', '100 tagacha foydalanuvchi', 'Ustuvor yordam'],
              highlight: false,
            },
          ].map((tier) => (
            <div
        ...
        {/* Курс сума живёт в uz/Pricing.tsx — здесь только ссылка, чтобы не дублировать его третий раз. */}
        <p className="mt-5 text-sm leading-relaxed text-white/45">
          Narxlar evroda koʻrsatilgan.{' '}
          <a
            href="/uz#tarify"
          ...
          valyutani almashtirib, soʻmdagi taxminiy summalarni koʻrishingiz mumkin. Har bir tarifda
          14 kunlik bepul demo-davr bor.
        </p>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-20 text-center md:px-12 md:py-28">
        <Reveal>
          <SectionTag>Keyingi qadam</SectionTag>
...
export default function UzModulePage() {
  const { slug } = useParams<{ slug: string }>()
  const data = slug ? UZ_MODULE_PAGES[slug] : undefined

  if (!data) return <Navigate to="/uz" replace />

  return (
    <ContactProvider>
      <UzModulePageContent data={data} />
    </ContactProvider>
  )
}
