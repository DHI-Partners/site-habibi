...
/* Adapted from the Motiq "Kinetic Grid" component to be *container-scoped*
   (the original is a full-screen background: fixed canvas, window-sized,
   window-level listeners). Here the canvas is sized to — and clipped by —
   its own wrapper, and the pointer is measured relative to that wrapper, so
   the effect can live inside a card. The warp/ripple/node algorithm itself
   is unchanged. */
...
export default function KineticGrid({
  ...
  useEffect(() => {
    ...
    // Pointer measured relative to the container so the warp reacts as the
    // cursor approaches the card. Listen on window so movement outside the card
    // still feeds a (far, sub-threshold) target.
