...
/** Счётчик клиентов одного тарифа: −/+ и ползунок. */
function Counter({
  label,
  price,
  value,
    ...
    <div className="rounded-2xl border border-white/12 bg-gradient-to-b from-white/[0.07] via-white/[0.025] to-transparent p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] transition-colors duration-300 hover:border-white/25">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-white/45">{formatMoney(price)} в месяц</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            type="button"
            aria-label={`Убрать клиента ${label}`}
          ...
          <button
            type="button"
            aria-label={`Добавить клиента ${label}`}
            onClick={() => onChange(clamp(value + 1))}
            className="flex h-9 w-9 items-center justify-center rounded-full border border-white/15 text-white/70 transition-colors hover:border-white/35 hover:text-white active:scale-95 disabled:opacity-30"
      ...
      <input
        ...
        max={MAX_PER_TIER}
        value={value}
        aria-label={`Клиентов на тарифе ${label}`}
...
export default function EarningsCalculator() {
  // Стартовая конфигурация — пример из копирайта: 5 клиентов на Habibi Pro.
        ...
        <div className="flex flex-col justify-center">
          ...
          >
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-sm text-white/55">Твоя ставка</span>
              <span
                key={rate}
            ...
            <div className="mt-6">
              <p className="text-sm text-white/55">Твой доход в месяц</p>
              <p
                key={monthly}
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 border-t border-white/10 pt-5">
              <div>
                <p className="text-xs text-white/45">В год</p>
                <p className="mt-1 text-lg font-medium text-white">{formatMoney(yearly)}</p>
              </div>
              <div>
                <p className="text-xs text-white/45">Выручка клиентов</p>
                <p className="mt-1 text-lg font-medium text-white">{formatMoney(mrr)} / мес</p>
              </div>
            </div>
          </div>
