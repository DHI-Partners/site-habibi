  ...
  return (
    <>
      ...
      {/* Мобильное меню. h-dvh (не h-screen) — на Android/Chrome 100vh считает
          высоту БЕЗ адресной строки. Контент не центрируем (justify-start) и
          делаем прокручиваемым (overflow-y-auto): пунктов меню + переключателя
          языков + кнопки CTA больше, чем помещается на экране телефона, и при
          вертикальном центрировании верх списка (языки) наезжал на логотип
          шапки, «сливаясь» с ним. */}
      <div
      ...
      >
        ...
        >
          <div className="mb-8 w-fit">
            <LangSwitcher current="ru" variant="inline" />
          </div>
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
